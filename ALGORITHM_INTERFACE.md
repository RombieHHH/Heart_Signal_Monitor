...
> 当前 develop 分支已完成应用集成，入口为 `ECGApp_Init/ECGApp_Poll`，
> 连续采样流水线为 `ECGMonitor_Push`。下面保留 alg 分支的模块接口说明；
> 当前应用的时序、质量门控、报警和上板验收以 `ACCEPTANCE.md` 为准。
> QRSDetector 返回能量包络峰，ECGMonitor 还会回溯定位显示波形的 R 极值。
...
默认预处理包含 0.7--25 Hz 显示通道和 50 Hz 陷波、5--15 Hz QRS 通道、0.5 s 削顶/平直窗口、突变与输入范围标志，以及启动后 2 s 学习期。所有样本都会进入滤波器，质量标志只用于门控，不删除原始数据。滤波系数按 500 Hz 设计；约 475--525 Hz 的常见误差可以直接使用，偏差更大时应重新生成系数。
