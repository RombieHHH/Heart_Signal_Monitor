...
static void LCD_DrawBrightness(uint8_t brightness_level)
{
    static const char *const numbers[4] = {"1", "2", "3", "4"};
    uint8_t i;
    uint16_t background;
    uint16_t foreground;
    ...
    for (i = 0U; i < 4U; ++i)
    {
        background = (i < brightness_level) ? LCD_COLOR_GREEN : 0x2104U;
        foreground = (i < brightness_level) ? LCD_COLOR_BLACK : LCD_COLOR_WHITE;
        LCD_FillRect((uint16_t)(18U + i * 56U), 172U, 44U, 42U, background);
        LCD_DrawString((uint16_t)(34U + i * 56U), 184U, numbers[i], 2U,
                       foreground);
    }
}

static void LCD_ShowInterface(void)
{
    LCD_Fill(LCD_COLOR_BLACK);
...
static void LCD_UpdateMode(LED_Mode mode)
{
    LCD_FillRect(18U, 78U, 204U, 42U, LCD_COLOR_BLACK);
    LCD_DrawString(18U, 84U, LED_GetModeName(mode), 3U, LCD_COLOR_YELLOW);
}

void LCDTaskFunc(void *argument)
{
    LED_Mode displayed_mode = LED_MODE_COUNT;
    uint8_t displayed_brightness = 0U;
    ...
    for (;;)
    {
        current_mode = LED_GetMode();
        current_brightness = LED_GetBrightnessLevel();

        if (current_mode != displayed_mode)
        {
            LCD_UpdateMode(current_mode);
            displayed_mode = current_mode;
        }
        if (current_brightness != displayed_brightness)
        {
            LCD_DrawBrightness(current_brightness);
            displayed_brightness = current_brightness;
        }
